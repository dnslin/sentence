...
type Variant = (typeof variants)[VariantId]
type VariantProps = { variant: Variant }

const variantIds = Object.keys(variants) as VariantId[]
const variantRenderers: Record<VariantId, ComponentType<VariantProps>> = {
  "quiet-gallery": QuietGallery,
  "immersive-stage": ImmersiveStage,
  "paper-desk": PaperDesk,
}

function getSelectedVariant(values: string[]): VariantId {
  if (values.length !== 1) {
...
export function PrototypeExperience() {
  ...
  const selectedId = getSelectedVariant(searchParams.getAll("variant"))
  const variant = variants[selectedId]
  const VariantView = variantRenderers[selectedId]
...
function QuietGallery({ variant }: VariantProps) {
  return (
...
function ImmersiveStage({ variant }: VariantProps) {
  return (
    <article aria-label="Immersive Stage 舞台原型" className="grid gap-6 rounded-[2rem] bg-stone-950 p-5 text-stone-50 shadow-xl lg:grid-cols-[1fr_20rem]">
      ...
      <aside className="flex flex-col justify-between gap-8 rounded-[1.5rem] border border-white/15 p-6">
        ...
        <div className="text-sm text-stone-300" aria-label="舞台边缘动作区">
...
function PaperDesk({ variant }: VariantProps) {
  return (
...
function VariantNotes({ variant, inverted = false }: VariantProps & { inverted?: boolean }) {
  return (
    ...
    >
      ...
      <div className="grid gap-2">
        {variantIds.map((id) => (
          ...
          >
            <Link href={`/prototype?variant=${id}`} aria-current={selectedId === id ? "page" : undefined}>
              {variants[id].name}
            </Link>
          </Button>
        ))}
